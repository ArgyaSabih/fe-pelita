...
export default function ModalApprovePermission({ isOpen, onClose, onApprove, permissionData }) {
  ...
  const handleApprove = () => {
    if (permissionData) {
      onApprove(permissionData, adminNotes);
      setAdminNotes(""); // Reset notes after approve
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        {/* Modal Header */}
        <h2 className="mb-4 text-2xl font-bold text-gray-900">Approve Surat Izin</h2>

        {/* Modal Body */}
        <div className="mb-6">
          <p className="mb-4 text-gray-700">Apakah Anda yakin ingin menyetujui surat izin ini?</p>
          ...
          {/* Admin Notes Input */}
          <div>
            <label htmlFor="adminNotes" className="mb-2 block text-sm font-medium text-gray-700">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className="cursor-pointer rounded-md bg-green-500 px-4 py-2 text-white transition-colors hover:bg-green-600"
          >
            Approve
          </button>
        </div>
